...
class Index extends Component {
  componentWillMount() {
    ...
    dispatch({
      type: 'report/getOnTimeCompletedProjects',
    });
  }
  ...
  render() {
    const {dispatch, report} = this.props;
    const {onTimeCompletedProjects} = report;
    ...
    const columns = [
      {
        title: 'TR',
        dataIndex: 'id',
        render: (text, record) => <a>{(onTimeCompletedProjects.indexOf(record) + 1)}</a>
      },
      {
        title: 'Loyiha nomi',
        dataIndex: 'name',
    ...
    const print = () => {
      let w = window.open();
      w.document.write(document.getElementById('report-inProgressProjects').innerHTML);
      w.print();
      w.close();
    };
    ...
    //   })
    // };
    let sum = onTimeCompletedProjects.reduce((prev, cur) => prev + cur.price, 0)
    return (
        ...
        <Row className='mt-3'>
          <Col md={8} className=''>
            <Card className='CardLarder-report'>
              <Row>
                <Col span={24}>
                  <h5 style={{cursor: "pointer"}} className='laderText mb-0'>O'z vaqtida tugatilgan loyihalar </h5>
                </Col>
              </Row>
              <Row className="pt-2 laderAllSumText-row">
                <Col span={20}>
                  <h6 className='mb-0 laderAllSumText'>O'z vaqtida tugatilgan loyihalarning umumiy summasi</h6>
                </Col>
              </Row>
              <Row>
        ...
        <Row className="mt-2">
          <Col>
            <h4 className="text-center about-report-responsive">
              O'z vaqtida tugatilgan loyihalar <br/> ro'yxati
            </h4>
          </Col>
        </Row>
        <Row className="mt-3 ">
          <Col span={24}>
            <Table className="completedProject-responsive" columns={columns} dataSource={onTimeCompletedProjects}/>
          </Col>
        </Row>
        <Row className="mt-5">
          <Col span={8} offset={16}>
            <div className="text-center">
              {/*<Button type="primary" className="export" onClick={downloadDebtClients}>Export Excel</Button>*/}
              <Button type="primary" className="ml-5 print" onClick={print}>Print</Button>
            </div>
          </Col>
        </Row>
        <div id="report-inProgressProjects" style={{display: 'none'}}>
          <Row className="mt-2">
            <Col>
              <h4 style={{textAlign: "center"}}>
                O'z vaqtida tugatilgan loyihalar
                <br/>
                RO'YXATI
              </h4>
            </Col>
          ...
          <table border="1" style={{borderCollapse: 'collapse', width: '100%'}}>
            ...
            <tr style={{backgroundColor: "#FAFAFA"}}>
              ...
              <th>Loyiha nomi</th>
              <th>Summasi</th>
              <th>Ochilgan sanasi</th>
              <th>Yakunlanga sanasi</th>
              <th>Tugatilish sanasi</th>
              <th>Yakunlanishi uchun ketgan kun</th>
            </tr>
            </thead>
            <tbody>
            {onTimeCompletedProjects.map((item, i) => <tr style={{textAlign: "center"}} key={i}>
              <td>{i + 1}</td>
              <td>{item.name}</td>
              <td>{item.price}</td>
            ...
            <tr style={{textAlign: "center"}}>
              <td></td>
              <td>Jami:</td>
              <td>{onTimeCompletedProjects.reduce((prev, cur) => prev + cur.price, 0)}</td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
