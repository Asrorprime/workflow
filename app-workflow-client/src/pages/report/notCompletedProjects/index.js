...
class Index extends Component {
  ...
  render() {
    ...
    const columns = [
      {
        title: 'TR',
        dataIndex: 'id',
        render: (text, record) => <a>{(notCompletedProjects.indexOf(record) + 1)}</a>
      },
      {
        title: 'Loyiha nomi',
        dataIndex: 'name',
    ...
    const print = () => {
      let w = window.open();
      w.document.write(document.getElementById('report-notCompletedProjects').innerHTML);
      w.print();
      w.close();
    };
    ...
    //   })
    // };
    let sum = notCompletedProjects.reduce((prev, cur) => prev + cur.price, 0)
    return (
      <div className="report">
        <Row className='mt-3'>
          <Col md={8} className=''>
            <Card className='CardLarder-report'>
              <Row>
                <Col span={24}>
                  <h5 style={{cursor: "pointer"}} className='laderText mb-0'>O'z muddatida yakunlamagan loyihalar </h5>
                </Col>
              </Row>
              <Row className="pt-2 laderAllSumText-row">
                <Col span={24}>
                  <h6 className='mb-0 laderAllSumText'>O'z muddatida yakunlanmagan loyihalar umumiy summasi</h6>
                </Col>
              </Row>
              <Row>
        ...
        <Row className="mt-2">
          <Col>
            <h4 className="text-center about-report-responsive">
              O'z muddatida yakunlanmagan loyihalar <br/> ro'yxati
            </h4>
          </Col>
        </Row>
        <Row className="mt-3 ">
          <Col span={24}>
            <Table className="completedProject-responsive" columns={columns} dataSource={notCompletedProjects}/>
          </Col>
        </Row>
        <Row className="mt-5">
        ...
        <div id="report-notCompletedProjects" style={{display: 'none'}}>
          <Row className="mt-2">
            <Col>
              <h4 style={{textAlign: "center"}}>
                O'z vaqtida yakunlanmagan loyihalar
                <br/>
                RO'YXATI
              </h4>
            </Col>
          ...
          <table border="1" style={{borderCollapse: 'collapse', width: '100%'}}>
            ...
            <tbody>
            {notCompletedProjects.map((item, i) => <tr style={{textAlign: "center"}} key={i}>
              <td>{i + 1}</td>
              <td>{item.name}</td>
              <td>{item.price}</td>
            ...
            <tr style={{textAlign: "center"}}>
              <td></td>
              <td>Jami:</td>
              <td>{notCompletedProjects.reduce((prev, cur) => prev + cur.price, 0)}</td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
