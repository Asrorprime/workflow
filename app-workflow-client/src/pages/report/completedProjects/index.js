...
class Index extends Component {
  componentWillMount() {
    ...
    dispatch({
      type: 'report/getCompletedProjects',
    });
  }
  ...
  render() {
    const {dispatch, report} = this.props;
    const {completedProjects} = report;
    ...
    const columns = [
      {
        title: 'TR',
        dataIndex: 'id',
        render: (text, record) => <a>{(completedProjects.indexOf(record) + 1)}</a>
      },
      {
        title: 'Loyiha nomi',
        dataIndex: 'name',
      ...
      },
      {
        ...
        key: 'projectId',
        render: text => <span>{getDifferenceDate(text)}</span>,
      },
    ];
    const print = () => {
      let w = window.open();
      w.document.write(document.getElementById('report-completedProjects').innerHTML);
      w.print();
      w.close();
    };
    const downloadCompletedProjects = () => {
      dispatch({
        type: 'report/downloadCompletedProjects',
        payload: download.makeForm(columns)
      })
    };
    let sum = completedProjects.reduce((prev, cur) => prev + cur.price, 0)
    return (
        ...
        <Row className='mt-3'>
          <Col md={8} className=''>
            <Card className='CardLarder-report'>
              <Row>
                <Col span={24}>
                  <h5 style={{cursor: "pointer"}} className='laderText mb-0'>Yakunlangan loyihalar </h5>
                </Col>
              </Row>
              <Row className="pt-2 laderAllSumText-row">
                <Col span={20}>
                  <h6 className='mb-0 laderAllSumText'>Yakunlangan loyihalar umumiy summasi</h6>
                </Col>
              </Row>
              <Row>
        ...
        <Row className="mt-2">
          <Col>
            <h4 className="text-center about-report-responsive">
              Yakunlangan loyihalar <br/> ro'yxati
            </h4>
          </Col>
        </Row>
        <Row className="mt-3 ">
          <Col span={24}>
            <Table className="completedProject-responsive" columns={columns} dataSource={completedProjects}/>
          </Col>
        </Row>
        <Row className="mt-5">
          <Col span={8} offset={16}>
            <div className="text-center">
              {/*<Button type="primary" className="export" onClick={downloadCompletedProjects}>Export Excel</Button>*/}
              <Button type="primary" className="ml-5 print" onClick={print}>Print</Button>
            </div>
          </Col>
        </Row>
        <div id="report-completedProjects" style={{display: 'none'}}>
          <Row className="mt-2">
            <Col>
          ...
          <table border="1" style={{borderCollapse: 'collapse', width: '100%'}}>
            ...
            <tbody>
            {completedProjects.map((item, i) => <tr style={{textAlign: "center"}} key={i}>
              <td>{i + 1}</td>
              <td>{item.name}</td>
              <td>{item.price}</td>
            ...
            <tr style={{textAlign: "center"}}>
              <td></td>
              <td>Jami:</td>
              <td>{completedProjects.reduce((prev, cur) => prev + cur.price, 0)}</td>
              <td></td>
              <td></td>
              <td></td>
              <td></td>
