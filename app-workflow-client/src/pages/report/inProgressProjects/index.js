...
class Index extends Component {
  componentWillMount() {
    ...
    dispatch({
      type: 'report/getInProgressProjects',
    });
  }
  ...
  render() {
    ...
    const columns = [
      {
        title: 'TR',
        dataIndex: 'id',
        render: (text, record) => <a>{(inProgressProjects.indexOf(record) + 1)}</a>
      },
      {
        title: 'Loyiha nomi',
        dataIndex: 'name',
    ...
    //   })
    // };
    let sum = inProgressProjects.reduce((prev, cur) => prev + cur.price, 0)
    return (
        ...
        <Row className='mt-3'>
          <Col md={8} className=''>
            <Card className='CardLarder-report'>
              <Row>
                <Col span={24}>
                  <h5 style={{cursor: "pointer"}} className='laderText mb-0'>Ish jarayonida bo'lgan loyihalar </h5>
                </Col>
              </Row>
              <Row className="pt-2 laderAllSumText-row">
                <Col span={20}>
                  <h6 className='mb-0 laderAllSumText'>Ish jarayonida bo'lgan loyihalar umumiy summasi</h6>
                </Col>
              </Row>
              <Row>
        ...
        <Row className="mt-2">
          <Col>
            <h4 className="text-center about-report-responsive">
              Ish jarayonida bo'lgan loyihalar <br/> ro'yxati
            </h4>
          </Col>
        </Row>
        <Row className="mt-3 ">
          <Col span={24}>
            <Table className="inProgressProject-responsive" columns={columns} dataSource={inProgressProjects}/>
          </Col>
        </Row>
        <Row className="mt-5">
        ...
        <div id="report-inProgressProjects" style={{display: 'none'}}>
          <Row className="mt-2">
            <Col>
              <h4 style={{textAlign: "center"}}>
                Ish jarayonida bo'lgan loyihalar
                <br/>
                RO'YXATI
              </h4>
            </Col>
          ...
          <table border="1" style={{borderCollapse: 'collapse', width: '100%'}}>
            ...
            <tr style={{backgroundColor: "#FAFAFA"}}>
              ...
              <th>Loyihaga berilgan kun</th>
            </tr>
            </thead>
            <tbody>
            {inProgressProjects.map((item, i) => <tr style={{textAlign: "center"}} key={i}>
              <td>{i + 1}</td>
              <td>{item.name}</td>
              <td>{item.price}</td>
            ...
            <tr style={{textAlign: "center"}}>
              <td></td>
              <td>Jami:</td>
              <td>{inProgressProjects.reduce((prev, cur) => prev + cur.price, 0)}</td>
              <td></td>
              <td></td>
              <td></td>
            </tr>
